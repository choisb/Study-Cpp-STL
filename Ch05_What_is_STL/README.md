...
- **STL의 구성요소**
  - **컨테이너**: 객체를 저장하는 객체로 컬렉션 혹은 자료구조라고도 한다.

  - **반복자**: 포인터와 비슷한 개념으로 컨테이너와 원소를 가리키고, 가리키는 원소에 접근하여
  다음 원소를 가리키게 하는 기능을 한다.

  - **알고리즘**: 정렬, 삭제, 검색, 연산 등을 해결하는 일반화된 방법을 제공하는 함수 템플릿이다.
  ...
  - **할당기**: 컨테이너의 메모리 할당 정책을 캡슐화한 클래스 객체로 모든 컨테이너는 자신만의 기본 할당기를 가진다. 
...
- **컨테이너의 사용 예시**
  - 예시로 `vector` 컨테이너를 사용한다.
    ...
    > 사용 예시는 소스코드 참고[(`ex05_01.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_01.cpp)
...
- **반복자의 사용 예시**
  > 사용예시는 소스코드 참고[(`ex05_02.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_02.cpp)
...
- **알고리즘의 사용 예시**
  ...
  아래에서 `find` 알고리즘과 `sort`알고리즘을 예를 들어 설명한다.
    ...
    > `find` 알고리즘 예시 [(`ex05_03.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_03.cpp)
    ```cpp
    ...
    >`sort` 알고리즘 예시 [(`ex05_04.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_04.cpp)
    ```cpp
        vector<int> v;
        v.push_back(10);
        v.push_back(20);
...
- **함수 객체 사용 예시**
  > `less`, `greater` 함수 객체 사용 예시 [(`ex05_05.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_05.cpp) 
    ```cpp
        ...
...
- **컨테이너 어댑터의 사용 예시**
  ...
  이용하는 `stack`을 구현한다.
  > `stack` 컨테이너 어댑터 [(`ex05_06.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_06.cpp)
    ```cpp
        ...
        // 일반적인 stack의 사용 방법 ////
...
- **반복자 어댑터의 사용 예시**
  - 대표적인 반복자 어댑터는 `revers_iterator`이다. `revers_iterator`어댑터는 다음과 같은 특징을 가진다.
    - 일반 반복자의 동작방식을 반대로 동작시키는 역방향 반복자로 변환한다.
      > 역방향 반복자 [(`ex05_07.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_07.cpp)
        ```cpp
            ...
            // 정방향으로 출력하는 일반 반복자
            for (vector<int>::iterator iter = v.begin(); iter != v.end(); ++iter)
    ...
    - 예제를 통해서 이를 살펴보도록 하자.
      > 정방향 반복자와 역방향 반복자가 가리키는 값[(`ex05_08.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_08.cpp)
        ```cpp
            int main()
            {
                vector<int> v;
...
- **함수 어댑터의 사용 예시**
  ...
  - `not1`은 단항 조건자에 `not2`는 이항 조건자에 사용된다.
    > 사용예시는 소스코드 참고[(`ex05_09.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_09.cpp)
...
- **할당기 사용 예시**
  ...
  > 컨테이너 기본 할당기 [(`ex05_10.cpp`)](https://github.com/choisb/Study-Cpp-STL/blob/master/Ch05_What_is_STL/ex05_10.cpp)
