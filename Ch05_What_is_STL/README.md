...
- **함수 객체 사용 예시**
  > `less`, `greater` 함수 객체 사용 예시 (`ex05_05.cpp`) 
    ```cpp
        ...
...
- **반복자 어댑터의 사용 예시**
  - 대표적인 반복자 어댑터는 `revers_iterator`이다. `revers_iterator`어댑터는 다음과 같은 특징을 가진다.
    ...
    - 예제를 통해서 이를 살펴보도록 하자.
      > 정방향 반복자와 역방향 반복자가 가리키는 값(`ex05_08.cpp`)
        ```cpp
            int main()
            {
                vector<int> v;
