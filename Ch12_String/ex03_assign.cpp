// =, assign() 사용 예제
// = 연산자는 전체 문자열을 할당할 떄 사용하며
// assign() 멤버 함수는 부분 문자열을 할당할 때 사용

// [출력 결과]
// s1.assign(s)Hello!
// s2.assign(s, off, n)Hello!
// s3.assign(sz)Hello!
// s4.assign(sz, n)Hello!
// s5.assign(n, c)HHHHHH
// s6.assign(iter1, iter2)Hello!
// s7.assign(p1, p2)Hello!
// s8 = sHello!
// s9 = szHello!

#include <iostream>
#include <string>
using namespace std;

int main()
{
	string t("Hello!");
	const char* p1 = "Hello!";
	const char* p2 = p1 + 6;

	string s1, s2, s3, s4, s5, s6, s7, s8, s9;

	s1.assign(t);
	s2.assign(t, 0, 6);
	s3.assign("Hello!");
	s4.assign("Hello!", 6);
	s5.assign(6, 'H');
	s6.assign(t.begin(), t.end());
	s7.assign(p1, p2);
	s8 = t;
	s9 = "Hello!";

	// sz: '\0' 문자열,  s: string 객체, c: 문자, off: 시작위치, n: 길이, iter: 반복자, p: 포인터
	cout << "s1.assign(s)" << s1 << endl;
	cout << "s2.assign(s, off, n)" << s2 << endl;
	cout << "s3.assign(sz)" << s3 << endl;
	cout << "s4.assign(sz, n)" << s4 << endl;
	cout << "s5.assign(n, c)" << s5 << endl;
	cout << "s6.assign(iter1, iter2)" << s6 << endl;
	cout << "s7.assign(p1, p2)" << s7 << endl;
	cout << "s8 = s" << s8 << endl;
	cout << "s9 = sz" << s9 << endl;

	return 0;

}